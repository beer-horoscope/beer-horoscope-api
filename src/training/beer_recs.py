...
def get_beer_recs_dataframe():
    ...
    beer_reviews_data_file = '{}/beer_reviews.csv'.format(TRAINED_MODELS_DIR)
    df = pd.read_csv(beer_reviews_data_file, low_memory=False)
    df = df.head(500000)
...
def load_inmemory():
    ...
    try:
        ...
        beer_names_list = []
        for beer in beer_names_list_from_pickle:
            beer_map = {}
            beer_map['beer_name'] = beer
            beer_names_list.append(beer_map)
